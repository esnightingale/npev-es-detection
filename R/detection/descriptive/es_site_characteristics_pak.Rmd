...
pacman::p_load(tidyverse, here, sf, janitor, terra, exactextractr, mgcv, gstat, broom)
theme_set(theme_minimal())

# Inputs
source(here("R/config.R"))
indir <- file.path(dir, "analysis")

# ES linelist
es <- readRDS(file.path(indir,"detection/es_analysis.rds")) |> 
  mutate(collection_month = month(collection_date),
         month = month(collection_date, label = T),
         year = year(collection_date),
         nonpolio = is.na(final_class)|final_class %in% c("NPEV","VACCINE"))

# AFP linelist
afp <- readRDS(file.path(indir,"prevalence/afp_analysis.rds"))

# Population raster
poprast  <- rast(file.path(dir, "WorldPop", "proj_pop_rast.tif"))
...
es |> 
  group_by(guid, admin_1,admin_2, site_id, site_name, x, y) |> 
  # Check when site established
  summarise(first_collection = min(collection_date),
            first_collection_y = year(first_collection),
            total_collections = n(),
            p_wpv = mean(final_class == "WPV1"),
            p_npev = mean(!is.na(npev))) -> es_sites

glimpse(es_sites)
table(year(es_sites$first_collection))

ggplot(es_sites, aes(x = first_collection_y)) + 
  geom_bar() + labs(x = "Year of first collection", y = "Sites")

```

In total, `r nrow(es_sites)` unique sites active at some point between
`r min(es$collection_date)` and `r max(es$collection_date)`. `r sum(es_sites$first_collection_y == 2024)` sites initiated in 2024, with a median of `r median(es_sites$total_collections[es_sites$first_collection_y == 2024])` and a maximum of `r max(es_sites$total_collections[es_sites$first_collection_y == 2024])` total collection(s). 

How many sites with <5 total collections?
```{r}

ggplot(es_sites, aes(total_collections)) +
  geom_histogram() + scale_x_continuous(trans = "log2")

table(cut(es_sites$total_collections, 
          breaks = c(0,1,4,9,19,49,max(es_sites$total_collections)),
          include.lowest = T))

```
What minimum sample size do we want per site? 
```{r}
...
Check uniqueness of coordinates per site code, and define sf:
```{r}

nrow(es_sites)
n_distinct(es_sites$site_id)

es_sites_sf <- st_as_sf(es_sites, coords = c("x","y"), crs = 4326)

```

Plot geographic coverage given sample size per site:
```{r}

ggplot() + 
  geom_sf(data = shape0 |> st_transform(4326)) + 
  geom_sf(data = es_sites_sf, aes(shape = "Site"), fill = "white") + 
  ...
  labs(shape = NULL,
       ...
       subtitle= "All sampled sites, 2021-2024")

ggplot() + 
  geom_sf(data = shape0 |> st_transform(4326)) + 
  geom_sf(data = filter(es_sites_sf, total_collections >= min_ss), 
          aes(shape = "Site"), fill = "white") + 
  scale_shape_manual(values = 23) + 
  theme(axis.text = element_blank(), 
        legend.position = c(0.8,0.2),
        legend.box.background = element_rect(fill = "white")) +
...
Quick look at WPV prevalence
```{r}

ggplot() + 
  geom_sf(data = shape0 |> st_transform(4326)) + 
  geom_sf(data = full_join(es_sites_sf, wpv_by_site) |> filter(total_collections >= min_ss) |> arrange(p_wpv), 
          aes(fill = p_wpv),
          pch = 22, alpha = 0.7) + 
  ...
  theme(axis.text = element_blank(), 
        ...
        legend.box.background = element_rect(fill = "white")) +
  labs(title = "WPV prevalence by site",
       fill = "Crude\nprevalence",
       subtitle = paste0("Sites with at least ",min_ss," sample(s), 2021-2024"))
...
sample(es_sites$site_name,20)

```

```{r}

mapview::mapView(es_sites_sf, col.regions = "gold", alpha.regions = 1)
...
es_sites |> 
  mutate(keywords = gsub("[^[:alnum:] ]", "", tolower(site_name))) |> separate_rows(keywords, sep = " ") |>
  group_by(keywords) |> 
  summarise(count = n()) |> 
  arrange(desc(count))
...
Relevant key words are pumping (station), drain/drainage, disposal.
...
es |> 
  mutate(site_name = tolower(site_name),
         ...
         wwtp = (grepl("pump", site_name) & 
                           grepl("station", site_name)) |grepl("plant", site_name),
...
```
### Exclude sites with few collections

```{r}

incl_sites <- es_sites$site_id[es_sites$total_collections >= min_ss]

es_sites <- filter(es_sites, site_id %in% incl_sites)
es_sites_sf <- filter(es_sites_sf, site_id %in% incl_sites)
es <- filter(es, site_id %in% incl_sites)

```

### Catchment population

#### Radius of site location

```{r}

es_sites_utm <- es_sites_sf |> 
  st_transform(proj_local)

es_buff <- st_buffer(es_sites_utm, dist = units::set_units(2, "km"))

# Population within 2km of each site:
es_sites$catchment_2k <- exactextractr::exact_extract(poprast, es_buff, "sum") 
es_sites_sf$catchment_2k <- es_sites$catchment_2k

# Total population per admin2 area:
shape2$total_pop <- exactextractr::exact_extract(poprast, shape2, "sum")
    
hist(es_sites$catchment_2k)
summary(es_sites$catchment_2k)

```

On average, `r mean(es_sites$catchment_2k, na.rm = T)` people reside
within 2km of an individual collection site, with a total of
`r sum(es_sites$catchment_2k)` within 2km of *any* site
(\~`r round(sum(es_sites$catchment_2k)/sum(shape2$total_pop)*100)`%
...
ggplot() + 
  geom_sf(data = shape0 |> st_transform(4326)) + 
  geom_sf(data = es_sites_sf, aes(size = catchment_2k, colour = catchment_2k), pch = 15, alpha = 0.7) + 
...
Import modelled watersheds from Novel-T
- How many (%) of sites linked to a watershed?
- Average area and population size?
...
es |> 
  group_by(month, year, site_id, x, y) |> 
  summarise(
    n_samples = n(),
...
ggplot() + 
  ...
  labs(fill = NULL,
       ...
       caption = paste0("Sites with at least ",min_ss," sample(s), 2021-2024"))
...
es |> 
  ...
  geom_hline(yintercept = 0.5, col = "grey", lty = "dashed") +
  geom_errorbar(width = 0.2) +
  geom_point() +
  ylim(c(0,1)) +
  labs(fill = "NPEV+", 
...
es |> 
  mutate(month_st = ymd(floor_date(collection_date, "month"))) |> 
  group_by(month_st) |>
  summarise(n = n(),
            npev_pos = sum(!is.na(npev)),
            lo = Hmisc::binconf(npev_pos, n)[2],
            hi = Hmisc::binconf(npev_pos, n)[3]) |>
  ungroup() |>
  ggplot(aes(month_st, npev_pos/n, ymin = lo, ymax = hi, col = (npev_pos/n > 0.5))) + 
...
ggplot() + 
  ...
  labs(fill = NULL,
       ...
       caption = paste0("At least ", min_ss," sample(s) collected Jan 2021-Dec 2024.")) +
  theme_void()
...
es |> 
  group_by(site_id, x, y, year, month) |> 
...
lapply(c(2021:2024), plot_es_ev_pos)
