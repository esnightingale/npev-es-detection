...
outdir <- "output/detection"
figdir <- "figures/es model"
...
Join district shapes to NPEV prevalence data: 
```{r}
...
### Test plots
Check obs and pred prevalence with some aggregated maps:
```{r}

plotdat <- npev_dist |> 
  group_by(guid) |> 
  summarise(npev_npafp_p = mean(npev_npafp_p),
            n_npafp = sum(n_npafp),
...
plotdat |> 
  ...
  geom_sf() + 
  scale_fill_viridis_b("Observed\nNPEV prevalence") + 
  labs(alpha = "No. tested")
plotdat |> 
  st_centroid() |> 
  ggplot(aes(col = npev_npafp_p, size = n_npafp)) + 
  geom_sf(data = shape2, fill = NA, inherit.aes = F) +
  geom_sf() + 
  scale_colour_viridis_c("Observed\nNPEV prevalence") + 
  labs(size = "No. tested")
plotdat |> 
...
```
## Site exclusion
Define a minimum number of samples to exclude low power sites:
...
tmp <- es |> 
   left_join(npev_prev |> 
               mutate(guid = guid_old) |> 
               select(guid, year, month, guid_pop_dens, pred_mean, pred_sd)) |> 
...
Seems some of the existing sample guids don't appear in district data? Re-do this link:
...
# Pivot built area by year for later join
sites_dist_xy_long <- sites_dist_xy |> pivot_longer(starts_with("built"), 
                                          names_prefix = "built",
                                          names_transform = \(x) as.numeric(x)+2000,
                                          names_to = "year",
                                          values_to = "built")
...
First intersect sites buffer with districts:
...
catch_5k_dist_max_long <- catch_5k_dist_max |> pivot_longer(starts_with("built"), 
                                          names_prefix = "built",
                                          names_transform = \(x) as.numeric(x)+2000,
                                          names_to = "year",
                                          values_to = "built")
...
catch_wsh_dist_max_long <- catch_wsh_dist_max |> pivot_longer(starts_with("built"), 
                                          names_prefix = "built",
                                          names_transform = \(x) as.numeric(x)+2000,
                                          names_to = "year",
                                          values_to = "built")
...
es |> 
  # Drop existing guid
  select(-guid) |> 
  left_join(sites_dist_xy_long) |>
  left_join(npev_prev |> select(guid, year, month, guid_pop_dens, pred_mean, pred_sd)) |> 
  select(guid, guid_pop_dens, site_id:coord_imp, 
         catchment_pop_5k, catchment_cat_5k, precip21, built,
         year, month, 
         pred_mean, pred_sd, everything()) -> es_npev_xy

es |> 
  # Drop existing guid
  select(-guid) |> 
  left_join(catch_5k_dist_max_long) |> 
  left_join(npev_prev |> select(guid, year, month, guid_pop_dens, pred_mean, pred_sd)) |> 
  select(guid, guid_pop_dens, site_id:coord_imp, catchment_pop_5k, catchment_cat_5k, 
         year, month,
...
#   # Drop existing guid
#   select(-guid) |> 
#   left_join(catch_wsh_dist_max_long) |> 
#   left_join(npev_prev |> select(guid, year, month, guid_pop_dens, pred_mean, pred_sd)) |> 
#   select(guid, guid_pop_dens, site_id:coord_imp, catchment_pop_5k, catchment_cat_5k, 
#          year, month,
...
```
How different are st_within and st_buffer joins?

```{r}

# For each site, how often is the matched district the same?

es_npev_xy |> 
  select(site_id, guid) |> distinct() |> 
  full_join(select(es_npev_5k, site_id, guid) |> distinct(), by = "site_id") |> 
  mutate(match = guid.x==guid.y) -> match_comp

mean(match_comp$match)
...
es_npev_xy |> 
  group_by(month, guid, pred_mean, pred_sd) |> 
  summarise(n_samp = n(),
            n_npev = sum(!is.na(npev)),
            p_npev = n_npev/n_samp) |> 
  arrange(n_samp) |> 
  ggplot(aes(p_npev, col = n_samp, size = n_samp,
             y = pred_mean)) + 
  geom_abline() +
  geom_jitter(alpha = 0.7) + 
  guides(size = "none") +
  scale_colour_viridis_b() + 
  labs(x = "Total ES positivity (across sites, per district/month)",
       y = "Predicted prevalence",
       col = "No. samples") + 
  theme(legend.position = c(0.85,0.8), legend.background = element_rect(fill = "white"))
ggsave(here(figdir,"es_prev_scatter.png"),
...
fitdata <- es_npev_xy |> 
  ...
  mutate(y = as.numeric(!is.na(npev)),
         ...
         built_s = scale(built),
         precip_s = scale(precip21))

site_ids <- unique(fitdata$site_id)

```

Baseline model with site- and time-specific random intercepts:
...
pp_base <- brm(f_base,
               ...
               file = here(outdir,"pp_es_base.rds"))
...
fit_base <- brm(f_base, 
               ...
               file_refit = "on_change",
               file = here(outdir,"fit_es_base.rds"))
...
```
Shift in detection across provinces
 - Punjab overall higher
 - Baloch. and Sindh more consistency across sites?
   -> This is because of all those singletons - no information to deviate from the average
...
Accounting for correlation between samples from the same site, there is an overall positive association between predicted prevalence and probability of ES detection (quite significantly so).
 - To what extent does this correlation vary between sites? 
...
# Redo this figure connecting site points - i.e. add link from unadjusted-adjusted (probably remove CIs)
summ_iid_site |> 
...
bayes_R2(fit2)
```
No correlation estimated between site-specific intercepts and slopes. Very little variability between slopes. 

Plot these random slopes...
...
f3 = bf(y ~ 0 + log_pop_5k_s + built_s + precip_s +
          me(pred_mean, pred_sd, gr = dist_t_id) +
          (1 | site_id))
