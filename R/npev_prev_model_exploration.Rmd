...
```
Adding AR reduces seasonality captured by the cyclic spline? 

Try a non_cyclic spline:
...
```
Not better than IID wrt R2 and temporal spline seems poorly identified? But does separate long term and seasonal trends from residual variability. 

### Long-term temporal spline by province {.tabset}
...
prior_bym_prov <- c(prior(normal(0,1), class = Intercept),
               prior(exponential(2), class = sd),
               # Tighten prior on spline since we're now fitting 7 different ones
...
fit_bym_prov <- update(fit_bym, 
                  ...
                  control = list(adapt_delta = 0.97),
                  recompile = T,
                  file = "../output/fit_bym_prov_st.rds") 
...
p1$plot$labels <- list(x = NULL, y = "Long-term temporal trend, by province")
...
p1$plot$data |> 
  ...
  labs(x = NULL, y = "Provincial deviation") -> p1_comb
...
fit_bym_prov_seas <- update(fit_bym_prov, 
                  ...
                  control = list(adapt_delta = 0.97),
                  recompile = T,
                  file = "../output/fit_bym_prov_seas.rds") 
...
fit_st <- add_criterion(fit_st, c("loo","waic"), moment_match = T)
...
loo_compare(fit_base, fit_bym, #fit_ar, 
            fit_st, fit_bym_prov, fit_bym_prov_seas, 
            criterion = "loo")
loo_compare(fit_base, fit_bym, #fit_ar, 
            fit_st, fit_bym_prov, fit_bym_prov_seas,
            criterion = "waic")
...
models <- list(fit_base, fit_bym, fit_ar, fit_st,
               fit_bym_prov, fit_bym_prov_seas)

lapply(models, \(x){bayes_R2(x) |> as.data.frame()}) |>
  setNames(c("Base","BYM","BYM_AR", "BYM_st",
             "BYM_prov","BYM_prov_seas")) |> 
