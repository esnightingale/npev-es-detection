...
Observed vs predicted prevalence of EV among AFP, per district and month.
```{r}
...
tmp |> 
  ggplot(aes(fill = pred_p, geometry = SHAPE)) + 
    geom_sf(col = NA) +
    facet_wrap(~month) +
    scale_fill_viridis_c(name = "Predicted EV+\nPrevalence") + 
...
saveRDS(pred, here("output/pred_prev_ev.rds"))

pred_draws_1000 <- pred |> 
  add_linpred_draws(fit, ndraws = 1000, transform = T) 
saveRDS(pred_draws, here("output/draws_prev_ev_1000.rds"))
