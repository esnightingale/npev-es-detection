...
afp |> 
  ...
  summarise(n = n(),
            ...
            p_ev = `EV+`*100/n) |>
  ungroup() |> 
  pivot_longer(`EV+`:`EV-`) -> plotdat_afp

ymax1 <- max(plotdat_afp$n)
ggplot(plotdat_afp) + 
  geom_col(aes(x = month, y = value,  fill = name)) +
  geom_line(aes(x = month, y = (p_ev / 100) * ymax1, group = 1),
            col = viridis::mako(5, end = 0.8)[3], lwd = 1) +
  scale_fill_manual(values = c("grey","grey50")) +
  scale_y_continuous(name = "Notified AFP cases",
                     sec.axis = sec_axis(trans=~ (. / ymax1) * 100,
                                         name = "% positive")) +
  theme(legend.position = c(0.2,0.9)) +
  labs(x = NULL, y = "Count", fill = NULL, 
...
es |> 
  ...
  summarise(n = n(),
            ...
            p_ev = `EV+`*100/n) |>
  ungroup() |> 
  pivot_longer(`EV+`:`EV-`) -> plotdat_es

ymax2 <- max(plotdat_es$n)
ggplot(plotdat_es) + 
  geom_col(aes(x = month, y = value,  fill = name)) +
  geom_line(aes(x = month, y = (p_ev / 100) * ymax2, group = 1),
            col = viridis::mako(5, end = 0.8)[3], lwd = 1) +
  scale_fill_manual(values = c("grey","grey50")) +
  scale_y_continuous(name = "Collected ES samples",
                     sec.axis = sec_axis(trans=~ (. / ymax2) * 100,
                                         name = "% positive")) +
  theme(legend.position = c(0.2,0.9)) +
  labs(x = NULL, y = "Count", fill = NULL,
...
ymax1 <- max(plotdat_afp$n, na.rm = TRUE)
ggplot(plotdat_afp) + 
  geom_col(aes(x = month_of_year, y = value,  fill = name)) +
  geom_line(aes(x = month_of_year, y = (p_ev / 100) * ymax1, group = 1),
            col = viridis::mako(5, end = 0.8)[3], lwd = 1) +
  scale_fill_manual(values = c("grey","grey50")) +
  scale_y_continuous(name = "Notified AFP cases",
                     sec.axis = sec_axis(trans=~ (. / ymax1) * 100,
                                         name = "% positive")) +
  theme(legend.position = c(0.2,0.8), legend.background = element_rect("white")) +
  labs(x = NULL, y = "Count", fill = NULL, 
       subtitle = "EV detection in AFP samples") -> p_afp_moy
...
ymax2 <- max(plotdat_es$n, na.rm = TRUE)
ggplot(plotdat_es) + 
  geom_col(aes(x = month_of_year, y = value,  fill = name)) +
  geom_line(aes(x = month_of_year, y = (p_ev / 100) * ymax2, group = 1),
            col = viridis::mako(5, end = 0.8)[3], lwd = 1) +
  scale_fill_manual(values = c("grey","grey50")) +
  scale_y_continuous(name = "Collected ES samples",
                     sec.axis = sec_axis(trans=~ (. / ymax2) * 100,
                                         name = NULL)) +
  theme(legend.position = c(0.2,0.8), legend.background = element_rect("white")) +
  labs(x = "Collection month", y = "Count", fill = NULL) -> p_es_moy
...
ev_site_yr |> 
  tbl_summary(include = c(n, ev, ev_p),
              by = year,
