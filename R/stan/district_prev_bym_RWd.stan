...
parameters {
  ...
  matrix[Nloc, Ntime] rw_d_raw;  // raw increments
  vector[Ntime] rw0;  // initial values for each time point (shared across districts)
  real<lower=0> sdrw_d;  // shared SD of random walks
}
transformed parameters {

  vector[Nloc] rcar;
  matrix[Nloc, Ntime] rw_d;
  ...
  // Priors for RW
  lprior += exponential_lpdf(sdrw_d | 1);
...
model {
 ...
 if (!prior_only) {
    ...
    for (n in 1:N) {
      mu[n] += rcar[Jloc[n]] + rw_d[Jloc[n], Jtime[n]];
    }
  ...
  target += std_normal_lpdf(to_vector(rw_d_raw));
  target += normal_lpdf(rw0 | 0, sdrw_d);
}
generated quantities {
  ...
  vector[N] linpred = Intercept + Zs_1_1 * s_1_1;
    for (n in 1:N) {
      ...
      linpred[n] += rcar[Jloc[n]] + rw_d[Jloc[n], Jtime[n]];
  ...
  // District-level RW deviations from overall seasonal trend
  matrix[Nloc, Ntime] rw_dev;
  for (t in 1:Ntime) {
    real mean_t = mean(to_vector(rw_d[, t]));
    for (d in 1:Nloc) {
      rw_dev[d, t] = rw_d[d, t] - mean_t;
    }
  }
}
