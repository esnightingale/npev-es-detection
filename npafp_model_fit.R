...
fitdata <- mutate(data, 
                  ...
                  guid_pop_dens_s = (guid_pop_dens - mean(guid_pop_dens))/sd(guid_pop_dens),
                  t_F = factor(t, levels = 1:max(data$t))) |> 
  arrange(adm1_name, guid, period) 
...
alt_fit <- brm(f_base, data = fitdata,
           ...
           adapt_delta = 0.95) ##, sample_prior="only")
